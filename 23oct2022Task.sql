...
SELECT*FROM INFORMATION_ABOUT WHERE Info_Father LIKE 'S%';
SELECT*FROM INFORMATION_ABOUT WHERE Info_Father LIKE 'N%';
SELECT*FROM INFORMATION_ABOUT WHERE Info_Father LIKE 'S%'; /*DISPLAYS NAME STARTING WITH S*/
SELECT*FROM INFORMATION_ABOUT WHERE Info_Father LIKE '%H'; /**DISPLAYS NAME ENDING WITH LAST LETTER H/
SELECT*FROM STATE_INFO WHERE CHIEF_MINISTER LIKE '%S%'; /DISPLAYS ANYTHING FROM THR TABLE WHICH CONTAINS S IN IT/
SELECT*FROM STATE_INFO WHERE CHIEF_MINISTER LIKE 'B%S%';  /*STARTS FRM B AND CONTAINS S AS WELL.AND THOSE TABLES WILL BE DISPLAYED */
SELECT*FROM INFORMATION_ABOUT WHERE Info_Father BETWEEN 'A' AND 'N'; /*DISPLAYS DATE HAVING A AND N*/

select*from STATE_INFO;
SELECT UPPER(Info_Elder_Name) FROM INFORMATION_ABOUT;
SELECT LOWER(Info_Elder_Name) FROM INFORMATION_ABOUT;
SELECT CONCAT(Info_Elder_Name,Info_Village_Name,Info_Height);
SELECT INSTR('INtrovertWorld','L')AS position;      /*DISPLAYS POS OF l IN THE WORD */
SELECT SUBSTR('Whats going buddy,,come lets have party tonight',2,9)AS substring;
SELECT SUBSTR(Info_Elder_Name,2,6) FROM INFORMATION_ABOUT;  /*DISPLAYS VALUES STARTING FROM INDEX 2 AND ENDING AT INDEX 6  FROM COLUMN CHIEF MINISTER */
